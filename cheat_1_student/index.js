...
const db_connection = "mongodb://127.0.0.1:27017/cheat-1"
const port =3000
...
app.get("/displayCountAndDocuments", async function (request, response) {
    const students = await StudentMarks.countDocuments()
    response.send({ "total count": students })
})
...
app.get("/getMoreThan25InAll", async function (request, response) {
    ...
    const html=`
    <ul>
    ${students.map(s1=>`<li>${s1.Name}</li>`).join("")}
    </ul>
    `
    response.send(html)
})
...
// i.  Remove specified student document from collecti
